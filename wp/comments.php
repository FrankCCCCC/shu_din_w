...
<section class="u-clearfix u-section-1" id="sec-a91c">
  <div class="u-clearfix u-sheet u-sheet-1" style="min-height: auto;">
	<div id="comments" class="comments-area">
		<?php if ( have_comments() ) : ?>
			<h3 class="comments-title">
				...
				if ( 1 === $comments_number ) {
					...
					printf( _x( 'One thought on &ldquo;%s&rdquo;', 'comments title', 'wp' ), get_the_title() );
				} else {
					printf(
				...
				$comments_html = preg_replace_callback('#class="comment-metadata([\s\S]*?)<\/div>#', function ($m) {
					...
					$result = str_replace(
						...
						array(
							...
							'</a></span>',
							'comment-metadata u-metadata u-metadata-1',
		...
		if ( ! comments_open() && get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) :
			?>
			<p class="no-comments "><?php _e( 'Comments are closed.', 'wp' ); ?></p>
		<?php endif; ?>
