...
function theme_customize_register($wp_customize) {
    ...
    $wp_customize->add_control('typography_heading_weight', array(
        'label' => __('Heading Weight', 'wp'),
        'section' => 'typography',
        'type' => 'select',
        'choices' => array(
            '100' => __('100 Thin', 'wp'),
            '300' => __('300 Light', 'wp'),
            '400' => __('400 Regular', 'wp'),
            '500' => __('500 Medium', 'wp'),
            '700' => __('700 Bold', 'wp'),
            '900' => __('900 Black', 'wp'),
        ),
    ));
    ...
    $wp_customize->add_control(new WP_Customize_Color_Control($wp_customize, 'logo_height', array(
        'type' => 'number',
        'label' => __('Logo max height (px)', 'wp'),
        'section' => 'title_tagline',
        'priority' => 9,
    )));
    ...
    $wp_customize->add_control(new WP_Customize_Cropped_Image_Control($wp_customize, 'custom_favicon', array(
        ...
        'width' => '64',
        'button_labels' => array(
            'select' => __('Select favicon', 'wp'),
            'change' => __('Change favicon', 'wp'),
            'remove' => __('Remove', 'wp'),
            'default' => __('Default', 'wp'),
            'placeholder' => __('No favicon selected', 'wp'),
            'frame_title' => __('Select favicon', 'wp'),
            'frame_button' => __('Choose favicon', 'wp'),
        ),
    )));
    ...
    $wp_customize->add_control('menu_trim_title', array(
        ...
        'label' => __('Trim long menu items', 'wp'),
    ));
    ...
    $wp_customize->add_control('menu_trim_len', array(
        'type' => 'number',
        'section' => 'menu_options',
        'label' => __('Limit each item to [N] characters', 'wp'),
    ));
    ...
    $wp_customize->add_control('submenu_trim_len', array(
        'type' => 'number',
        'section' => 'menu_options',
        'label' => __('Limit each subitem to [N] characters', 'wp'),
    ));
    ...
    $wp_customize->add_control('menu_use_tag_filter', array(
        'type' => 'checkbox',
        'section' => 'menu_options',
        'label' => __('Apply menu item tag filter', 'wp'),
    ));
    ...
    $wp_customize->add_control('menu_allowed_tags', array(
        'type' => 'text',
        'section' => 'menu_options',
        'label' => __('Allowed menu item tags', 'wp'),
    ));
    ...
    $wp_customize->add_control('use_default_menu', array(
        'type' => 'checkbox',
        'section' => 'menu_options',
        'label' => __('Use not stylized menu', 'wp'),
        'description' => __('Used standart wp_nav_menu when option is enabled (need for some third-party plugins).', 'wp'),
    ));
    ...
    $wp_customize->add_control('excerpt_auto', array(
        ...
        'label' => __('Use auto excerpts', 'wp'),
        'description' => __('Generate post excerpts automatically (When neither more-tag nor post excerpt is used)', 'wp'),
    ));
    ...
    $wp_customize->add_control('excerpt_words', array(
        'type' => 'number',
        'section' => 'excerpt_options',
        'label' => __('Excerpt length (words)', 'wp'),
    ));
    ...
    $wp_customize->add_control('excerpt_min_remainder', array(
        'type' => 'number',
        'section' => 'excerpt_options',
        'label' => __('Excerpt balance (words)', 'wp'),
    ));
    ...
    $wp_customize->add_control('excerpt_strip_shortcodes', array(
        'type' => 'checkbox',
        'section' => 'excerpt_options',
        'label' => __('Remove shortcodes from excerpt', 'wp'),
    ));
    ...
    $wp_customize->add_control('excerpt_use_tag_filter', array(
        'type' => 'checkbox',
        'section' => 'excerpt_options',
        'label' => __('Apply excerpt tag filter', 'wp'),
    ));
    ...
    $wp_customize->add_control('excerpt_allowed_tags', array(
        'type' => 'text',
        'section' => 'excerpt_options',
        'label' => __('Allowed excerpt tags', 'wp'),
    ));
    ...
    $wp_customize->add_control('morelink_template', array(
        'type' => 'text',
        'section' => 'excerpt_options',
        'label' => __('More Link Template', 'wp'),
        'description' => sprintf(__('ShortTags: %s', 'wp'), '[url], [text]'),
    ));
    ...
    $wp_customize->add_control('seo_og', array(
        'type' => 'checkbox',
        'section' => 'seo_options',
        'label' => __('Include Open Graph meta tags', 'wp'),
    ));
    ...
    $wp_customize->add_control('seo_ld', array(
        'type' => 'checkbox',
        'section' => 'seo_options',
        'label' => __('Include schema.org JSON-LD syntax markup', 'wp'),
    ));
    ...
    $wp_customize->add_control('include_jquery', array(
        ...
        'label' => __('Use jQuery from theme', 'wp'),
    ));
    ...
    $wp_customize->add_section('sidebar_options', array(
        'capability' => 'edit_theme_options',
        'title' => __('Sidebars', 'wp'),
        'priority' => 100,
    ));

    $layouts_choices = array(
        '' => __('Default', 'wp'),
        'left-sidebar' => __('Left', 'wp'),
        'right-sidebar' => __('Right', 'wp'),
        'none' => __('None', 'wp'),
    );
    ...
    $wp_customize->add_control('sidebars_layout_blog', array(
        ...
        'label' => __('Blog Template', 'wp'),
        'description' => __('Templates for displaying blogs, archives, search results', 'wp'),
    ));
    ...
    $wp_customize->add_control('sidebars_layout_post', array(
        ...
        'choices' => $layouts_choices,
        'section' => 'sidebar_options',
        'label' => __('Post Template', 'wp'),
        'description' => __('Templates for displaying single posts', 'wp'),
    ));
    ...
    $wp_customize->add_control('sidebars_layout_default', array(
        ...
        'choices' => $layouts_choices,
        'section' => 'sidebar_options',
        'label' => __('Default Template', 'wp'),
        'description' => __('Templates for displaying single pages and everything else', 'wp'),
    ));
}
add_action('customize_register', 'theme_customize_register', 11);

function theme_print_sidebars_blog_caption() {
    printf('<h3>%s</h3>', __('Display sidebars on:', 'wp'));
}
